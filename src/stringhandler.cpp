...
QMap<QString, QPair<QString, QString>> StringHandler::GetLocalValue(
    QString str, QMap<QString, QPair<QString, QString>> &res) {
...
QMap<QString, QPair<QString, QString>> StringHandler::GetLocalType(
    QString str, QMap<QString, QPair<QString, QString>> &res) {
  return res;
}
...
QList<QMap<QString, QString>> StringHandler::ToBreakPointInfo(QString str) {
  ...
  // QRegularExpression
  // exp1("(\d)\s+([a-z]+)\s+([a-z]+)\s+([a-z]+)\s+([a-z\d]+)\s+([a-z\s().:\d]+)");
  //    QRegularExpression
  //    exp1("(\\d)\\s+([a-z]+)\\s+([a-z]+)\\s+([a-z]+)\\s+([a-z\\d]+)\\s+([a-z\\s().:\\d]+?)\n|"
  //                            "(\\d)\\s+([a-z]+)\\s+([a-z]+)\\s+([a-z]+)\\s+([a-z\\d]+)\\s+([a-z\\s().:\\d]+)$");
  QRegularExpression exp1(
      "(\\d)\\s+([a-z]+)\\s+([a-z]+)\\s+([a-z]+)\\s+(0x[a-z\\d]+)\\s+([a-z\\s/"
      "()-A-Z:]+?..*\\d)");
...
QList<QString> StringHandler::FindBreakPoint(QString str) {
  QList<QString> res;
  // QRegularExpression exp("(Breakpoint|Temporary
  // breakpoint)\\s*(\\d+),\\s*(.*)at(.*):(\\d+)"); QRegularExpression
  // exp("(Breakpoint|Temporary
  // breakpoint)\\s*(\\d+?),\\s*(.*)at([.A-Z\\n\\s:/-\\da-z]+?):(\\d+)");
  QRegularExpression exp(
